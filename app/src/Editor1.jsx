  ...
  const offLoaded = window.electronAPI.onWorkflowLoaded(async (data) => {
    ...
    const restoredNodes = await Promise.all(
      data.nodes.map(async (n) => {
        let restoredData = { ...n.data, DATA_TYPES }; // DATA_TYPES を必ず再付与

        if (n.type === 'LoadImageNode') {
          restoredData.onUpload = handleImageUpload;
          restoredData.nodeId = n.id;

          if (restoredData.imagePath) {
            try {
              const buffer = await window.electronAPI.loadImageAsBlob(restoredData.imagePath);
              const blob = new Blob([new Uint8Array(buffer)]);
              restoredData.imageUrl = URL.createObjectURL(blob);
            } catch (err) {
              console.error('画像Blob復元失敗:', err);
            }
          }
        }
        ...
        if (n.type === 'layerImage') {
          restoredData.onImageUpload = handleImageUpload;
          restoredData.editorConfig = editorConfig;
          
          if (restoredData.layerImagePath) {
            try {
              const buffer = await window.electronAPI.loadImageAsBlob(restoredData.layerImagePath);
              const blob = new Blob([new Uint8Array(buffer)]);
              restoredData.imageUrl = URL.createObjectURL(blob);
            } catch (err) {
              console.error('レイヤー画像Blob復元失敗:', err);
            }
          }
        }
        ...
        if (n.type === 'SlideShowNode') {
          restoredData.images = (restoredData.images || []).map(img => ({
            ...img,
            imageUrl: img.imagePath
              ? `file://${img.imagePath.replace(/\\/g, '/')}`: null,
          }));
        }

        if (n.type === 'VideoSlideShowNode') {
          restoredData.videos = (restoredData.videos || []).map(v => ({
            ...v,
            videoUrl: v.videoPath
              ? `file://${v.videoPath.replace(/\\/g, '/')}`: null,
          }));
        }
        // ★重要：VisualNovelExportNode の getGraph を最新 refs で再設定
...
// 2. エッジ復元 + 色の再計算
    const restoredEdges = data.edges.map((e) => {
      // sourceノードを探して sourceHandle の dataType を取得
    ...
    // 3. ノベルノードのプレビューを強制再描画（これで即座に表示される）
    setNodes((current) => syncMediaNodes(current, restoredEdges));
  });
  return () => {
    offRequest?.();
    offLoaded?.();
...
// ★修正: LayerOnOffNodeからの出力をLayerPreviewNodeに同期（無限ループを防ぐ）
useEffect(() => {
  // layerOnOffノードを探す
